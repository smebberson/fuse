...
var EventEmitter = require('events').EventEmitter,
	...
	debugCommon = require('debug')('fuse:common'),
	debugCommonDetail = require('debug')('fuse:common-detail'),
	async = require('async');
...
Fuse.prototype.fuseFile = function (inputFile, outputFile) {
	...
	debugFile('Regex is %s', this.regex);
	...
	if (!matches.length && unwatched === false) {
		...
		debugFile('Fusing %s match' + (matches.length > 1 ? 'es' : ''), matches.length);
...
Fuse.prototype.fuseContent = function (content, relativePath, mode) {
	...
	debugContent('Regex is ' + this.regex);
	...
	debugContent('Fusing %s match' + (matches.length > 1 ? 'es' : ''), matches.length);
	debugContentDetail('Fuses are %s', matches.map(function (match) {
		return match.str;
	}).join(','));
...
Fuse.prototype.fuse = function (content, matches, relativePath, callback) {
	...
	async.eachSeries(matches, function (match, fn) {
		...
		debugCommon('Loaded and replaced content on file %s', filepath);
		debugCommonDetail('File %s content is\n%s', filepath, fileContent);
